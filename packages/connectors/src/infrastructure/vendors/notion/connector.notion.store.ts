...
export class ConnectorNotionStore implements IConnectorStore {
  ...
  async save<T extends NotionEntityType>(data: T | T[]): Promise<void> {
    const items = this._resolveItems(data);

    for (const item of items) {
      switch (item.__type) {
        case NOTION_ENTITY_TYPES_ENUM.PAGE:
          await this._connectorNotionRepository.page.savePage(item as unknown as NotionPageEntity, {
            incremental: false,
          });
          break;
        default:
  ...
  private _resolveItems<T extends NotionEntityType>(data: T | T[]): T[] {
