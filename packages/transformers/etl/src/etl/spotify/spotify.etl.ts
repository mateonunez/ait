...
import { generateEmbedding } from "../../infrastructure/embeddings/etl.embeddings.service";
...
export class SpotifyTrackETL implements IBasicETL {
  ...
  private readonly _vectorSize = 384; // Size of embedding vectors
  ...
  private async _transform(tracks: SpotifyTrackDataTarget[]): Promise<VectorPoint[]> {
    console.log(`Transforming ${tracks.length} tracks`);
    try {
      const points = tracks.map((track, index) => {
        const vector = generateEmbedding(track.name);
  ...
  private async _load(points: VectorPoint[]): Promise<void> {
    console.log(`Loading ${points.length} vector points into Qdrant in batches of ${this._batchSize}`);
    ...
    try {
      ...
      console.log(`Successfully loaded all ${points.length} vector points into Qdrant`);
    } catch (error: any) {
  ...
  private async retry<T>(
    ...
    try {
      ...
      console.log(`Retry ${retryCount + 1}/${this._retryOptions.maxRetries} after ${delay}ms`);
